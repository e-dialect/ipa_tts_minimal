...
def load_checkpoint(model, ckpt_path, device: str, dtype=None, use_ema=True, lora=None):
    ...
    if use_ema:
        ...
        # delete dialect_classifier
        for key in ["transformer.moe.dialect_classifier.weight", "transformer.moe.dialect_classifier.bias"]:
            if key in checkpoint["model_state_dict"]:
                del checkpoint["model_state_dict"][key]
        model.load_state_dict(checkpoint["model_state_dict"])
    if lora is not None:
...
def load_model(
    ...
    model = CFM(
        transformer=model_cls(**model_cfg, text_num_embeds=vocab_size, mel_dim=n_mel_channels, moe=moe),
...
def infer_process_single(
    ...
    # print("input text: ", final_text_list)
    ...
    with torch.inference_mode():
        ...
        dit_rtf = t_dit / (duration * hop_length / target_sample_rate)
        vocoder_rtf = t_vocoder / (duration * hop_length / target_sample_rate)
        ave_rtf = (t_dit + t_vocoder) / (duration * hop_length / target_sample_rate)

    return generated_wave, target_sample_rate, dit_rtf, vocoder_rtf, ave_rtf


def infer_process_single_ori(
    ref_audio,
    ref_text,
    gen_text,
    model_obj,
...
def infer_process(
    ...
    for i, gen_text_in in enumerate(gen_text_batches):
        print(f"gen_text {i}", gen_text_in)
    print("\n")
...
def infer_batch_process(
    ...
    def process_batch(gen_text):
        ...
        if fix_duration is not None:
            ...
            # ref_text_len = len(ref_text.encode("utf-8"))
            # gen_text_len = len(gen_text.encode("utf-8"))
            ref_text_len = len(ref_text.split(' '))
            gen_text_len = len(gen_text.split(' '))

            duration = ref_audio_len + int(ref_audio_len / ref_text_len * gen_text_len / local_speed)
        # inference
        # MoE
        if model_obj.use_moe:
            gate_mask_len = ref_text_len
        else:
            gate_mask_len = None
        with torch.inference_mode():
            generated, _ = model_obj.sample(
